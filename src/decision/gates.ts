...
import type { Candidate, CompetitionState, GroupMetrics, GateResult, MarkoutSummary, RewardUniverse } from '../types.ts';
...
export function evaluateGates(ctx: GateContext): { passed: GateResult[]; failed: GateResult[] } {
  ...
  push('pair-fill-count', ctx.group.fillCount >= ctx.cfg.minPairFillCount,
    'fills=' + ctx.group.fillCount + ' min=' + ctx.cfg.minPairFillCount);
  ...
  push('gross-denominator', ctx.group.grossGroupFillUsd > 0,
    'grossGroupFillUsd=' + ctx.group.grossGroupFillUsd.toFixed(2));
